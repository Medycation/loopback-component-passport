 ...
 * @property {String} [authCallback] The callback for handling passport.authenticate. Arguments are 
 * `err, user, messages, context`
 *
...
PassportConfigurator.prototype.configureProvider = function (name, options) {
  ...
  if (authType === 'local') {
    ...
    var passportOptions = options.fn || _.defaults({
      ...
      self.app.post(authPath, function (req, res, next) {
        passport.authenticate(
          ...
          , function (err, user, message) {
            ...
            options.authCallback(err, user, message, context);
          }
        )(req, res, next);
      })
    } else {
